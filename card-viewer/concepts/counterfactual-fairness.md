...
## 문제
성별을 $A=1$로 개입했을 때 이 개인의 우편번호는 $Z_{A\leftarrow 1}=\beta\times 1+u=0.4+u$이고 예측값은 $\hat Y_{A\leftarrow 1}=\gamma(0.4+u)=0.5\times(0.4+u)=$ ==빈칸== 이다.

## 해설
0.5를 0.4와 $u$ 각각에 분배해서 곱하면 $0.2+0.5u$가 되기 때문이에요.
