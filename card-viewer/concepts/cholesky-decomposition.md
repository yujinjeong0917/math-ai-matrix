...
## 도입
다변량 정규분포 $N(\mu,\Sigma)$에서 표본을 뽑고 싶다면, 표준정규 난수는 쉽게 만들 수 있지만 성분끼리 상관된 표본은 그렇지 않습니다. 대칭 양의정부호 행렬 $\Sigma$를 하삼각행렬의 제곱 형태로 쪼갤 수 있다면, 독립적인 표준정규 표본에 이 하삼각행렬을 곱하는 것만으로 원하는 상관구조를 그대로 만들어낼 수 있습니다.
