...
## 문제
두 확률의 비를 쓰면 $\dfrac{p_i}{p_j}=\dfrac{\exp(z_i/T)/\sum_k\exp(z_k/T)}{\exp(z_j/T)/\sum_k\exp(z_k/T)}$인데, 분모의 정규화 합 $\sum_k\exp(z_k/T)$가 분자·분모 모두에 똑같이 들어있어 약분되고 지수법칙을 쓰면 ==빈칸== 만 남는다.

## 해설
정규화 상수가 약분되어 사라지고 exp(z_i/T)를 exp(z_j/T)로 나눈 값은 지수법칙에 의해 exp(z_i/T - z_j/T) = exp((z_i-z_j)/T)로 정리돼요.

**정답: $\exp\!\big((z_i-z_j)/T\big)$**
