...
## 문제
최상위층 $h$는 노드가 상수 개(약 1개) 남을 때까지 층을 쌓은 것이므로 $n\,p^h \approx 1$이 성립한다. 양변에 로그를 취하고 $h$에 대해 정리하면 $h =$ ==빈칸== 이다.

## 해설
n p^h=1의 양변에 로그를 취하면 log n + h log p=0이 되고, h에 대해 정리하면 h=(-log n)/log p, 즉 log_{1/p} n이 돼요.
