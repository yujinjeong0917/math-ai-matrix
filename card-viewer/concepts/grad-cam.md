...
## 문제
$y^c$를 특징맵의 한 위치 $A^k_{ij}$로 미분하려면 체인룰을 써서 $\frac{\partial y^c}{\partial A^k_{ij}} = \frac{\partial y^c}{\partial F^k}\cdot\frac{\partial F^k}{\partial A^k_{ij}}$로 쪼갠다. $y^c=\sum_k w_k^c F^k$에서 $F^k$ 하나만 남기고 나머지 채널의 항은 $F^k$와 무관한 상수이므로 미분하면 사라진다. 따라서 $\frac{\partial y^c}{\partial F^k} = $==빈칸== 만 남는다.

## 해설
y^c는 F^k에 대해 계수가 w_k^c인 일차식이에요. 그래서 F^k로 미분하면 다른 채널의 항은 다 사라지고 그 계수인 w_k^c만 남아요.
