...
## 문제
$\bar g$의 정의에 $g_i$를 그대로 대입하면 $\bar g=\dfrac{1}{N}\sum_{i=1}^N\left(\dfrac{1}{b}\sum_{j=1}^b\nabla\ell(\theta;x_{ij})\right)$이다. 두 합 기호 앞의 상수를 하나로 묶으면 $\bar g=$==빈칸==$\sum_{i=1}^N\sum_{j=1}^b\nabla\ell(\theta;x_{ij})$ 로 정리된다.

## 해설
1/N과 1/b를 곱하면 1/(Nb)가 되고, 이 상수를 이중합 앞으로 묶어낼 수 있어요.
