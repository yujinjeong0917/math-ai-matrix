...
## 문제
$C_t$를 $C_{t-1}$의 함수로 볼 때, 두 번째 항 $i_t\odot\tilde C_t$는 $C_{t-1}$을 포함하지 않으므로 이를 $C_{t-1}$에 대해 미분하면 $0$이 된다. 첫 번째 항은 $C_{t-1}$에 어떤 계수가 곱해진 선형식이므로, $aX$를 $X$에 대해 미분하면 계수 $a$만 남는다는 규칙을 적용해야 한다. 이때 곱해지는 계수는 s1의 갱신식에서 $C_{t-1}$ 앞에 붙어 있던 게이트값이므로, $\dfrac{\partial C_t}{\partial C_{t-1}}=$==빈칸== 이다.

## 해설
C_{t-1}에 곱해지는 계수가 s1의 갱신식 C_t=f_t⊙C_{t-1}+i_t⊙\tilde C_t에서 f_t였으므로, aX를 X로 미분하면 계수 a만 남는다는 규칙에 따라 미분값은 f_t가 되기 때문이에요.
