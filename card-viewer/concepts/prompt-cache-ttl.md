...
## 문제
두 번째 요청부터, $k$번째 요청($k\ge2$)이 직전 요청의 캐시를 그대로 히트로 재사용하려면 직전 요청과의 간격 $\tau$가 TTL을 넘지 않아야 한다. 즉 $k$번째 요청이 히트가 되기 위한 조건은 ==빈칸== 이다.

## 해설
TTL은 마지막 사용 이후 경과 시간이 그 한도를 넘지 않아야 캐시가 만료되지 않는다는 정의이므로, 요청 간격 $\tau$가 TTL $T$ 이하여야 캐시가 살아있기 때문이에요.
