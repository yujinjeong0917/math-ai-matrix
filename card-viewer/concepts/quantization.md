...
## 문제
역양자화 식 $\hat x=s(q-z)$에 $q=\mathrm{round}(x/s)+z$를 대입하면 오차는 $e=x-\hat x=x-s\big(\mathrm{round}(x/s)+z-z\big)=$==빈칸== 이다.

## 해설
더해졌던 영점 z가 그대로 빠지는 z와 상쇄되어 사라져요. 오차의 크기는 영점 위치와 무관하고 오직 반올림 자체에서만 생긴다는 것을 보여줘요.

**정답: $x-s\,\mathrm{round}(x/s)$**
