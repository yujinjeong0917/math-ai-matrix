...
## 도입
CG는 대칭성 덕분에 작동합니다: $A$-켤레 방향 사이의 짧은 재귀관계도, 잔차의 직교성도 모두 $A=A^T$라는 사실에 기대고 있습니다. $A$가 비대칭이면 $A$-켤레 방향을 정의하는 내적 $u^TAv$ 자체가 대칭적인 좋은 성질(양의정부호성과 결합된 대칭)을 잃어버리고, 짧은 재귀관계를 보장하는 증명이 무너집니다. GMRES는 목표를 '켤레 방향 찾기'에서 'Krylov 부분공간 안에서 잔차를 최소로 만드는 벡터 찾기'로 바꾸어 이 문제를 우회합니다.
