...
## 문제
임의의 실수 $a_1,\dots,a_n$에 대해 삼각부등식 $\left|\sum_{i=1}^n a_i\right| \le \sum_{i=1}^n |a_i|$가 항상 성립한다. 여기에 $a_i = \phi_j^{(i)}$를 대입하고 양변을 $n$으로 나누면 ==빈칸== 라는 부등식을 얻는다.

## 해설
삼각부등식 양변을 n으로 나누면 좌변은 부호 그대로 평균 낸 값의 절댓값인 |φ̄_j|가 되고, 우변은 절댓값의 평균인 I_j가 되어 이 부등식이 그대로 유도돼요.
