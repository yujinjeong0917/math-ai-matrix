...
## 문제
모든 이동의 비용은 0 아니면 1이므로 어떤 정렬의 총비용도 음수가 될 수 없고 따라서 최소비용도 항상 0 이상이다. 최소비용을 최악비용으로 나눈 값도 0 이상이므로 $1 - (\text{최소비용}/\text{최악비용})$이 가질 수 있는 최댓값은 ==빈칸==이다.

## 해설
0 이상인 값을 1에서 빼면 결과는 1보다 커질 수 없기 때문이에요.
