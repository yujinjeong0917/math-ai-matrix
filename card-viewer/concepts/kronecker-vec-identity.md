...
## 도입
행렬 하나를 통째로 다루는 대신 길게 편 벡터로 바꿔서 다루고 싶을 때가 있어요. 배치 연산이나 텐서 라이브러리에서 이런 변환은 아주 흔합니다. 그런데 $AXB$처럼 행렬 $X$를 양쪽에서 다른 행렬로 감싼 식을 벡터화하면 단순히 $X$를 늘어놓은 벡터에 뭘 곱해야 나올까요. 답은 크로네커 곱이에요. 이 항등식 하나로 복잡한 다중선형연산이 표준적인 행렬-벡터 곱 하나로 정리됩니다.
