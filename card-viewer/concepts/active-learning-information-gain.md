...
## 도입
라벨링 예산이 정해진 상황에서 다음에 어떤 샘플에 라벨을 붙일지 고르는 문제가 능동학습입니다. 그저 모델이 헷갈려 하는 샘플(예측 엔트로피가 큰 샘플)을 고르면 될 것 같지만, 그 헷갈림이 "데이터 자체의 애매함(관측 잡음)"인지 "모델이 파라미터를 아직 못 정해서 생기는 헷갈림"인지 구분하지 못하면 잘못된 샘플을 고르게 됩니다. BALD(Bayesian Active Learning by Disagreement)는 이 둘을 상호정보로 정확히 분리합니다.
