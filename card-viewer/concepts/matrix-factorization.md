...
## 문제
먼저 $L_{ui}$를 $p_u$에 대해 미분합니다. 제곱 오차항 $e_{ui}^2$의 미분은 연쇄법칙에 의해 $2e_{ui}$에 $e_{ui}=r_{ui}-p_u^\top q_i$를 $p_u$로 미분한 결과를 곱한 값이고, $e_{ui}$를 $p_u$로 미분하면 $-q_i$가 남습니다. 정규화 항의 미분은 $2\lambda p_u$이므로 전체 그래디언트는 $\dfrac{\partial L_{ui}}{\partial p_u}=$==빈칸== 입니다.

## 해설
제곱항의 미분 2e_ui·(-q_i)와 정규화항의 미분 2λp_u를 더하면 전체 그래디언트가 나와요.

**정답: $-2e_{ui}q_i+2\lambda p_u$**
