...
## 문제
예시에서 추천 목록 크기는 $|\text{Recommended}|=20$이고 유용하다고 반응한 아이템은 $|\text{Useful}|=8$이었다. 정확도 지표인 정밀도(precision)를 $\mathrm{Precision}=|\text{Useful}|/|\text{Recommended}|$로 정의하면 이 값은 $\mathrm{Precision}=8/20=$==빈칸==이다.

## 해설
유용한 아이템 8개를 전체 추천 20개로 나누면 정밀도가 나와요.
