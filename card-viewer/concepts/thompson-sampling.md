...
## 문제
베이즈 정리에 의해 사후분포는 사전분포와 우도의 곱에 비례하므로 $p(\theta_a\mid \text{data}) \propto \theta_a^{\alpha_a-1}(1-\theta_a)^{\beta_a-1}\cdot \theta_a^{n_1}(1-\theta_a)^{n_0}$ 이다. 같은 밑의 지수를 더해 정리하면 이 식은 ==빈칸== 형태가 된다.

## 해설
지수 α_a-1과 n_1을 더하면 (α_a+n_1)-1이 되고, β_a-1과 n_0을 더하면 (β_a+n_0)-1이 되어 베타분포의 표준형과 같은 꼴이 나와요.

**정답: $\theta_a^{(\alpha_a+n_1)-1}(1-\theta_a)^{(\beta_a+n_0)-1}$**
