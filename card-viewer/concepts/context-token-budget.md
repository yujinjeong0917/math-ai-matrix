...
## 문제
예산 부등식은 $T_{\text{system}} + T_{\text{history}} + T_{\text{retrieval}} + T_{\text{output}} \le T_{\text{max}}$ 이다. 모델의 최대 토큰이 $T_{\text{max}}=8000$ 이고 시스템 프롬프트에 $T_{\text{system}}=500$ 을, 답변 출력을 위해 미리 $T_{\text{output}}=2000$ 을 남겨둔다고 하자. 이 두 조각을 먼저 배정하고 나면 대화 이력과 검색 결과가 함께 나눠 쓸 수 있는 토큰의 상한은 $T_{\text{history}} + T_{\text{retrieval}} \le$ ==빈칸== 이다.

## 해설
전체 한도 8000에서 시스템 프롬프트 500과 출력 예비분 2000을 미리 빼고 남는 몫이 이력과 검색 결과가 나눠 쓸 수 있는 전체 한도이기 때문이에요.

**정답: $8000-500-2000=5500$**
