...
## 문제
각 팀에 대해 $u_i(t) \le q_i$가 항상 성립한다고 할 때, 이를 $i=1$부터 $n$까지 모두 더하면(부등식은 양변끼리 더해도 방향이 유지되므로), ==빈칸== 을 얻습니다.

## 해설
부등식 u_i(t) ≤ q_i가 모든 i에 대해 성립한다고 가정했으므로, 좌변끼리 우변끼리 각각 더해도 부등식의 방향은 유지되어 합이 성립해요.

**정답: $\sum_{i=1}^n u_i(t) \le \sum_{i=1}^n q_i$**
