...
## 문제
이미지의 세로 길이가 $H$, 가로 길이가 $W$, 패치 한 변의 길이가 $p$라고 하자. 가로 방향으로는 $W/p$개, 세로 방향으로는 $H/p$개의 겹치지 않는 패치가 만들어지므로 전체 패치 개수는 $N=$==빈칸==이다.

## 해설
격자 형태로 배열된 패치의 전체 개수는 세로 방향 칸 수와 가로 방향 칸 수를 곱해야 나오기 때문이에요.
