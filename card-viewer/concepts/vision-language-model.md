...
## 문제
행렬곱 $XW_Q$가 정의되려면 $X$의 열 개수와 $W_Q$의 행 개수가 일치해야 한다. 따라서 $X$의 각 행, 즉 시퀀스에 들어가는 모든 벡터의 차원은 ==빈칸== 이어야 한다. 그런데 이미지 벡터 $v_i$의 차원은 $d_v$이고 일반적으로 $d_v\ne d_{model}$이므로 $v_i$를 그대로 $X$의 한 행으로 끼워 넣으면 이 조건이 깨진다.

## 해설
s1에서 $W_Q\in\mathbb{R}^{d_{model}\times d_{model}}$로 정의했으므로 그 행 개수는 d_model이고, 행렬곱이 정의되려면 X의 열 개수 즉 시퀀스 벡터의 차원도 이와 같아야 해요.
