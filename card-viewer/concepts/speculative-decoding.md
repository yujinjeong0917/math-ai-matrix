...
## 문제
초안 토큰으로 $x$가 제안되고 그대로 받아들여질 확률은 $q(x)\alpha(x)=q(x)\min(1,p(x)/q(x))$이다. $q(x)>0$이므로 이를 정리하면 $q(x)\alpha(x)=$==빈칸== 이다.

## 해설
$q(x)$를 $\min(1,p(x)/q(x))$ 안으로 곱해 넣으면 두 값 $q(x)\cdot1$과 $q(x)\cdot p(x)/q(x)=p(x)$ 중 작은 값을 취하는 것과 같아지기 때문이에요.
