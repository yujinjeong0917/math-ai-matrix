...
## 도입
분류 문제에서 정답 라벨은 보통 정답 클래스만 $1$이고 나머지는 모두 $0$인 원-핫 벡터로 주어져요. 그런데 이 원-핫 목표를 정말로 완벽하게 맞히려면 모델은 정답 클래스의 로짓을 다른 클래스보다 한없이 크게 밀어붙여야 해요. 그러다 보면 모델은 점점 과신하는 방향으로만 학습되고 확률 보정은 나빠져요. 라벨 스무딩은 원-핫 목표를 균등분포 쪽으로 살짝 섞어서 이 문제를 막아요. 그 결과 손실이 최소가 되는 지점에서 로짓 차이가 정확히 어디까지만 벌어질 수 있는지 유한한 상한이 생긴다는 걸 확인해 봅니다.
