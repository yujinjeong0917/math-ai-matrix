...
## 문제
겹침 $o$가 있는 경우, 두번째 청크의 시작 위치는 첫번째 청크의 시작 위치에서 스트라이드만큼 이동한 자리다. 첫번째 청크가 $B-s$에서 시작해 $B$에서 끝난다고 하면(길이가 $s$이므로), 두번째 청크의 시작 위치는 $(B-s) + \mathrm{stride} = (B-s)+(s-o)$이고, 이를 정리하면 ==빈칸== 이다.

## 해설
(B-s)+(s-o)에서 s가 상쇄되어 B-o만 남아요.
