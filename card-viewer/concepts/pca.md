...
## 도입
데이터를 한 방향으로 압축해서 보고 싶습니다. 그런데 정보를 가장 많이 남기는 방향은 어떻게 고를까요. PCA의 답은 단순해요. 데이터가 가장 넓게 퍼진 방향을 고르면 돼요. 분산이 가장 큰 방향을 고르는 거예요.

이걸 식으로 옮겨볼게요. 단위벡터 $w$ 방향으로 데이터를 눌러봤을 때(사영했을 때) 그 결과가 얼마나 넓게 퍼지는지는 $w^T\Sigma w$로 계산돼요. 여기서 $w^T$는 $w$를 가로로 눕힌 것(전치, transpose)이에요. 벡터를 세로로 쓴 것과 가로로 쓴 것을 곱하면 숫자 하나가 나오는데, 왜 하필 이렇게 곱해야 "퍼진 정도"가 나오는지는 아래 증명에서 직접 확인해볼 거예요. 지금은 "$w^T\Sigma w$ = $w$ 방향으로 데이터를 눌렀을 때의 분산"이라는 것만 기억해두면 충분해요.

그리고 그 방향은 공분산행렬의 고유벡터 중 하나로 정확히 떨어집니다.
