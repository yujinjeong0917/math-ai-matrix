...
## 문제
먼저 Guided Backprop의 경우를 보자. 순위 차이는 $d_i = r_i - r_i'$ 로 정의되는데, 두 순위가 모든 픽셀에서 완전히 같으므로 $d_i =$ ==빈칸== 가 모든 $i$에 대해 성립한다. 따라서 $\sum_i d_i^2 =$ [[blank:나]] 이다.

## 해설
두 saliency map의 순위가 모든 픽셀에서 동일하므로 그 차이는 0이 돼요.
