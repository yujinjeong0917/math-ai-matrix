...
## 문제
패치는 이미지의 왼쪽 위부터 겹치지 않게 순서대로 놓입니다. 너비 $W$가 패치 크기 $p$로 정확히 나누어떨어지지 않으면, 마지막에 남는 픽셀 부분도 패치 한 칸으로 처리해서 이미지 전체를 빠짐없이 덮어야 합니다. 이 조건을 만족하는 가로 방향 패치 조각 수는 $n_w=$==빈칸== 입니다.

## 해설
나눗셈이 딱 떨어지지 않을 때 남는 픽셀도 패치 한 칸으로 세어 이미지 전체를 덮으려면 몫을 올림한 값을 써야 하기 때문이에요.
