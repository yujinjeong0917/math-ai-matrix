...
## 문제
인접한 두 순위 $i$번째와 $i+1$번째에 각각 관련도 $rel_i, rel_{i+1}$인 아이템이 있다고 하자. 이 두 자리가 DCG에 기여하는 부분은 $\dfrac{rel_i}{\log_2(i+1)} + \dfrac{rel_{i+1}}{\log_2(i+2)}$ 이다. 만약 이 둘의 순서를 맞바꾸면 기여분은 ==빈칸== 로 바뀐다.

## 해설
자리를 맞바꾸면 앞자리(로그값이 작은 쪽)엔 rel_{i+1}이, 뒷자리엔 rel_i가 오게 되어 분자만 서로 자리를 바꾼 값이 돼요.

**정답: $\dfrac{rel_{i+1}}{\log_2(i+1)} + \dfrac{rel_i}{\log_2(i+2)}$**
