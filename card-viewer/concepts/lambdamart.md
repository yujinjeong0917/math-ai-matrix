...
## 문제
$u = -\sigma(s_i-s_j)$로 두면 $C_{ij} = \log(1+e^{u})$ 꼴이 된다. 로그함수와 지수함수의 합성이므로 연쇄법칙의 앞부분을 계산하면 $\dfrac{dC_{ij}}{du} = $==빈칸== 이다.

## 해설
log(1+e^u)를 u로 미분하면 합성함수 미분법으로 분자에 e^u, 분모에 1+e^u가 그대로 남는 표준 형태가 나와요.
