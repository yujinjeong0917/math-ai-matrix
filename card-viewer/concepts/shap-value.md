...
## 문제
이 순열 표현이 원래의 부분집합 합 공식과 정확히 같은 값이 되려면, 크기가 $s=|S|$인 고정된 집합 $S\subseteq N\setminus\{i\}$에 대해 $P_i^\pi=S$가 되는 순열 $\pi$가 몇 개인지 세어봐야 한다. $S$의 원소들을 $i$ 앞자리에 배치하는 순서는 $s!$가지이고, 남은 $n-1-s$개의 원소를 $i$ 뒷자리에 배치하는 순서는 $(n-1-s)!$가지이므로, 이 둘을 곱하면 $P_i^\pi=S$인 순열의 개수는 ==빈칸== 가 된다. 이를 전체 순열의 수 $n!$로 나누면 원래 정의의 가중치 $\frac{s!(n-s-1)!}{n!}$와 정확히 일치한다.

## 해설
S를 i 앞에 배치하는 s!가지 경우와 나머지를 i 뒤에 배치하는 (n-1-s)!가지 경우가 동시에 일어나야 하므로 두 경우의 수를 곱해야 해요.
