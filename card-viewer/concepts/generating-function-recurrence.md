...
## 도입
점화식을 하나씩 손으로 풀어나가는 대신, 수열 전체를 $A(x)=\sum_n a_n x^n$이라는 하나의 함수(생성함수)로 묶어버리면, 점화식은 $A(x)$가 만족하는 대수 방정식으로 바뀝니다. 이 방정식을 풀어 $A(x)$의 닫힌 형태를 구한 뒤, 부분분수분해로 다시 계수 $a_n$을 뽑아내면 점화식의 명시적인 해가 나옵니다.
