...
## 문제
표준이 없을 때는 도구 하나와 프레임워크 하나의 짝마다 하나씩 별도의 연동 코드가 필요하다. 도구가 $M$개, 프레임워크가 $N$개이고 이 둘의 짝을 빠짐없이 만드는 경우의 수를 세어보면, 표준이 없을 때 필요한 연동 코드의 총 개수는 ==빈칸==개다.

## 해설
도구 $M$개와 프레임워크 $N$개의 모든 짝을 하나씩 세면 곱의 법칙에 따라 $M\times N$가지가 되기 때문이에요.
