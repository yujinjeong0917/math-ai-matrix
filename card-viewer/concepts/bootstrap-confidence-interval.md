...
## 도입
MLE의 점근정규성처럼 표집분포를 분석적으로 유도하기 어려운 통계량도 많습니다(중앙값, 상관계수, 복잡한 함수 등). 부트스트랩은 알 수 없는 모분포 $F$ 대신, 표본이 만든 경험분포 $\hat F_n$에서 복원추출로 재표집(resampling)해 통계량의 표집분포를 컴퓨터로 근사합니다. "경험분포가 모분포를 잘 근사한다"는 사실 하나에 기대어, 분석적 유도 없이도 신뢰구간을 만들 수 있다는 것이 핵심 아이디어입니다.
