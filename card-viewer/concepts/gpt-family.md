...
## 문제
양변에 자연로그를 취하고 부호를 반대로 하면, 시퀀스 전체의 음의 로그우도 $L=-\ln p(x_1,x_2,x_3,x_4)$는 각 위치의 음의 로그우도 $L_i=-\ln p(x_i\mid x_1,\ldots,x_{i-1})$의 합으로 다시 쓸 수 있다. 즉 $L=$==빈칸== 이다.

## 해설
곱의 로그는 로그의 합이라는 성질을 이어 적용하면, 결합확률의 음의 로그우도가 각 위치별 음의 로그우도의 단순 합으로 정확히 분해돼요.
