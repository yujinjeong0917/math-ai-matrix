...
## 문제
행렬곱 $QK^T$에서 $Q\in\mathbb{R}^{t\times d_k}$이고 $K^T\in\mathbb{R}^{d_k\times m}$이므로 그 곱 $QK^T$의 크기는 ==빈칸== 이고 이 행렬 하나를 계산하는 데 필요한 곱셈 횟수는 $t\times d_k\times m$에 비례한다.

## 해설
행렬곱의 결과 크기는 앞 행렬의 행 개수와 뒤 행렬의 열 개수로 정해지는데 Q의 행이 t개, K^T의 열이 m개이므로 결과는 t행 m열 크기가 돼요.
