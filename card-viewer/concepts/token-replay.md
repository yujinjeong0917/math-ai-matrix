...
## 문제
$m \ge 0$이므로 $\dfrac{m}{c} \ge 0$이고 따라서 $1 - \dfrac{m}{c} \le 1$이다. 같은 방식으로 $r \ge 0$이므로 $1 - \dfrac{r}{p} \le 1$이다. 두 항에 각각 계수 $\dfrac12$를 곱하면 각 항은 $\dfrac12$를 넘을 수 없으므로 이 둘을 더한 fitness가 넘을 수 없는 최댓값은 ==빈칸==이다.

## 해설
각 항의 상한이 1/2이므로 두 항을 더한 값은 1을 넘을 수 없기 때문이에요.
