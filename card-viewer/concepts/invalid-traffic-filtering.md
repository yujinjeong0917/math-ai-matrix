...
## 문제
유효 트래픽 비율을 $v$라 하면 전체 트래픽은 유효와 무효로 완전히 나뉘므로 $v + \mathrm{IVT\ Rate} = 1$이다. 여기에 $\mathrm{IVT\ Rate}=0.085$를 대입하면 $v=$==빈칸== 이다.

## 해설
1에서 무효 트래픽 비율 0.085를 빼면 유효 트래픽이 차지하는 비율이 나와요.
