...
## 도입
큰 희소 대칭행렬 $A$의 고유값을 구하고 싶을 때, QR 알고리즘처럼 $A$ 전체를 밀집행렬로 다루며 대각화하는 것은 비용이 너무 큽니다. 대신 $A$와의 곱셈(행렬-벡터 곱)만 반복해서 사용할 수 있다면, 작은 Krylov 부분공간 안에서 $A$의 극단적인 고유값들을 훨씬 싸게 근사할 수 있습니다. Lanczos 반복법은 대칭행렬 $A$를 정규직교기저 위에서 삼중대각행렬로 압축하는 절차이고, 대칭성 덕분에 매 스텝마다 직전 두 벡터만 기억하면 충분한 짧은 3항 재귀관계로 이 압축이 이루어집니다.
