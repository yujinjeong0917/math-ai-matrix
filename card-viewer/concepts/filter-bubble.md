...
## 문제
합 $\sum_{i \ne j \in R} d(i,j)$은 서로 다른 클러스터에 속한 순서쌍만 $1$을 기여하고 나머지는 $0$을 기여한다. $i \in A, j \in B$인 순서쌍이 $n_A n_B$개, $i \in B, j \in A$인 순서쌍이 $n_A n_B$개이므로, 이 합을 모두 세면 ==빈칸== 이다.

## 해설
서로 다른 클러스터 조합은 순서를 바꾼 두 가지 경우($i\in A,j\in B$와 $i\in B,j\in A$)로 각각 $n_A n_B$번씩 나타나기 때문에 둘을 더해야 전체 순서쌍 개수가 나와요.
