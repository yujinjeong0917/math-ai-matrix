...
## 문제
$g^*(z') := \beta_0+\sum_k \beta_k z_k'$라는 특정 선형모델을 후보로 놓고 손실을 계산해보자. 교란 샘플에서는 $z'=z$로 두므로, $f(z)-g^*(z') = \big(\beta_0+\sum_k\beta_k z_k\big) - \big(\beta_0+\sum_k\beta_k z_k\big) = $==빈칸== 가 모든 교란 샘플에서 성립한다. 이를 손실식 $\mathcal{L}(f,g,\pi_x)=\sum_{z,z'}\pi_x(z)(f(z)-g(z'))^2$에 대입하면 $\mathcal{L}(f,g^*,\pi_x) = \sum_{z,z'}\pi_x(z)\cdot 0^2 = $[[blank:나]] 이다.

## 해설
g*=f로 두면 두 식이 완전히 동일해지므로 그 차를 계산하면 남는 것 없이 정확히 0이 되기 때문이에요.
