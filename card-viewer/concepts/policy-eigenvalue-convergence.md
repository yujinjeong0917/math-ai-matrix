...
## 도입
정책평가 반복 $V_{k+1}=R+\gamma PV_k$은 벨만 기대연산자를 그냥 반복 적용하는 것뿐인데, 몇 번 반복해야 답에 충분히 가까워지는지는 감이 잘 오지 않습니다. 오차가 매 스텝 얼마나 줄어드는지는 반복행렬 $\gamma P$의 스펙트럴반경(고유값 크기의 최댓값)이 정확히 결정합니다.
