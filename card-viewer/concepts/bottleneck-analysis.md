...
## 문제
먼저 세 구간의 평균 대기시간을 모두 더해 케이스 하나가 접수부터 완료까지 걸리는 평균 총 소요시간을 구한다. $0.4 + 4.2 + 0.3 =$==빈칸== 일이다.

## 해설
세 구간의 평균 대기시간을 그대로 더하면 되는데, 각 구간이 전체 사이클을 겹치지 않게 나눈 구간이므로 이 합이 곧 접수부터 완료까지의 평균 총 소요시간이 돼요.
