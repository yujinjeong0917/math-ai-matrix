...
## 문제
XOR 분기에서 $x, y$가 배타적이라는 조건, 즉 모든 트레이스 $\sigma \in L$에 대해 $x\in\sigma$이면 $y\notin\sigma$가 성립한다고 하자. 이제 귀류법으로 $x>_L y$가 성립한다고 가정하면, $a>_L b$의 정의를 그대로 적용했을 때 어떤 트레이스 $\sigma$안에 ==빈칸==가 성립해야 한다.

## 해설
$a>_Lb$의 정의를 $x,y$에 그대로 적용하면 어떤 위치 $i$에서 $\sigma(i)=x, \sigma(i+1)=y$가 성립해야 해요.

**정답: $\sigma(i)=x,\ \sigma(i+1)=y$**
