...
## 문제
타일 하나하나는 인코더가 학습된 고정 해상도로 독립적으로 인코딩되므로, 타일 한 장이 만들어내는 토큰 수 $T$는 타일 개수 $n$과 무관하게 항상 같은 값입니다. 따라서 $n$장의 타일 전체가 만들어내는 토큰 수는 ==빈칸== 입니다.

## 해설
타일마다 똑같이 $T$개의 토큰이 나오는데 그런 타일이 $n$개 있으므로, $T$를 $n$번 더한 것과 같은 $nT$가 되기 때문이에요.
